...
- **Live Screen Stream**: Background capture loop (Rust + DirectML) emits a constant PNG feed to the Tauri UI so you can watch ChronoLog think in real time.
...
- **Semantic Search UI**: Ask natural-language questions from the desktop client; results stream back immediately even while the capture loop continues.
...
1. **Recall Desk UI** – Persistent vertical carousel of search results, floating detail pane, and fullscreen viewer so results stay visible while capture continues.
...
3. **Privacy Filters** – Default blurred faces in thumbnail previews with an "uncloak" toggle; full-resolution originals stay intact for face-based search.
...
5. **Contributor Playground** – Documented hooks so collaborators can prototype custom overlays, agents, or robotic workflows on top of the memory stream.
...
- [ ] **Phase 1 — Recall Desk** *(in progress)*
    ...
    - [ ] Thumbnail face-blur overlays and manual uncloaking.
- [ ] **Phase 2 — Canvas**
